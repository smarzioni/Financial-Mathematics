...
int main()
{
	...
	vector<Wrapper<StatisticsMC>> VectGatherers;
	StatisticsMean meanGatherer;
	VectGatherers.push_back(meanGatherer);
	FourMoments momentsGatherer;
	VectGatherers.push_back(momentsGatherer);
	ValueAtRisk VaRGatherer(0.95);
	VectGatherers.push_back(VaRGatherer);
	SeveralStats gatherers(VectGatherers);

	Counter counter(NumberOfPaths);
	...
	vector<vector<double>> results = gatherers.GetResultsSoFar();
